package param;

public class PathParam {
    //文件根路径
    public static String rootPath="D:\\GitProject\\";

    //Git项目路径
    public static String gitProjectPath=rootPath+"GitClone\\";

    //ZIP路径
    public static String zipPath=rootPath+"ZipFiles\\";

    //flow文件名
    public static String flowName="azkaban.flow";

    //zip包名
    public static String zipName= "zip4Azkaban";

    //jar包名
    public static String azkabanJarName= "project.jar";

    //yaml文件名
    public static String ymlName="azkaban.flow";

    //version文件名
    public static String versionPath=zipPath+"version.project";

    //html路径
    public static final String VIEW_EDIT_HTML_PATH="az-view-commit\\src\\web\\index.html";

    //chrome.exe路径
    public static final String CHROME_PATH="D:\\Azkaban\\hanrb\\ChromeDriver\\chromedriver.exe";
}
