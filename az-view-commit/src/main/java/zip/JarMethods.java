...
public class JarMethods {
    private static final Logger log = Logger.getLogger(JarMethods.class);
    ...
    public long getJarSize(){
        ...
        if (file.exists() && file.isFile()) {
            log.info("文件"+jarName+"的大小是："+file.length());
            return file.length();
        }
        else{
            log.info("jar包不存在");
    ...
    public boolean renameJar(){
        ...
        if (file.renameTo(new File(jarPath+ PathParam.azkabanJarName))) {
            log.info("重命名jar包成功! newName："+PathParam.azkabanJarName);
